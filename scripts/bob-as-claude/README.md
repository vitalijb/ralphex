...
Suppressed events (init header, user-message echo, non-terminal tool calls) are emitted as empty keepalive deltas so a configured `idle_timeout` does not kill a healthy session during a long silent tool run. bob's stderr is merged into the same stream as stdout and re-emitted as `content_block_delta` events so ralphex's error/limit pattern detection works; any literal `<<<RALPHEX:` token in diagnostics is neutralized to `<<< RALPHEX:` (a space is inserted) so stray diagnostics cannot be mistaken for a real completion signal.
...
- `BOB_REVIEW_GUARD` — controls how review mode blocks nested agent CLI invocations. Values: `path` (default, cheap PATH shims), `bwrap` (stronger sandbox isolation if `bwrap` is installed), `none` (disable the guard).
...
- **Nested agent CLI guard.** Review mode (`ralphex-review`) optionally places `bob`, `claude`, and `codex` guard shims on `PATH` (default) or runs `bob` under `bwrap` when `BOB_REVIEW_GUARD=bwrap`. This blocks the model from emulating parallel sub-agents by spawning nested agent CLIs, which can exhaust provider limits or deadlock. Absolute-path launches bypass the default PATH guard; use `bwrap` for stronger isolation. Disable with `BOB_REVIEW_GUARD=none` if your workflow requires nested agent launches.
- **bob version drift.** The wrapper is tested against bob 1.0.6. If bob changes its stream-json schema or removes `-m`/`--model`, the event parser will need updating. The `fromjson?` guard and non-JSON fallback prevent hard crashes, but translation would silently produce no output for unrecognized event types.
...
- **stderr signal neutralization.** The wrapper re-emits bob's merged stdout+stderr diagnostics as `content_block_delta` events so ralphex's error/limit pattern detection works. Any literal `<<<RALPHEX:` token in diagnostics is neutralized to `<<< RALPHEX:` (space inserted) so stray diagnostics cannot be mistaken for a real completion signal. Rate-limit and `API Error:` phrases pass through verbatim for error/limit detection.
