...
- `BOB_SHELL` — shell bob's `execute_command` runs commands through (default: `bash`). Resolved with `command -v`, so a bare name or an absolute path both work; an unresolvable value fails the run at startup. See [Shell pinning](#shell-pinning) below.
...
Plan runs buffer token-level assistant `message` deltas and detect boundaries from that text alone; reasoning messages (`isReasoning: true`) are excluded. `QUESTION` payloads must be a valid JSON object with a non-empty `question` string and a non-empty array of non-empty `options`, and `PLAN_DRAFT` must be non-empty. Among the valid boundaries in the buffer, one standing at the start of a line outranks a mid-sentence one — the plan prompt teaches the model these exact tokens, so it may name a marker in prose before emitting the real boundary — and the earliest wins within the same class. A terminal marker (`PLAN_READY`, `TASK_FAILED`) quoted inside a still-open `QUESTION`/`PLAN_DRAFT` body is treated as narration, not a boundary. The boundary is emitted as one Claude text delta, Bob is terminated, and trailing autonomous output is discarded. If Bob exits without any complete valid boundary, the wrapper reports an error and exits non-zero instead of silently starting another plan iteration.
...
### Transient backend failures are retried

When bob fails from a cause a re-run can plausibly clear — a backend 5xx, `socket hang up`, `ECONNRESET`, `ETIMEDOUT`, `fetch failed` — the wrapper emits a bare `BOB_TRANSIENT_ERROR` line as its last output. That token is in ralphex's default `claude_retry_patterns`, so the phase retries the iteration after a short backoff instead of failing the run. Without it a single backend hiccup aborts the whole run mid-review, since bob's message matches none of ralphex's stock patterns.
