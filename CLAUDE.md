...
scripts/bob-as-claude/install-modes.sh # installs the shipped Bob modes (no other setup step)
...
- bob: `BOB_CHAT_MODE`, `BOB_MODEL` (accepted, ignored), `BOB_VERBOSE`, `BOB_EXTRA_ARGS`, `BOB_SHELL`
...
bob wrapper: targets bob v2 only (>= 2.0.0; no version-detection branch, no v1 compatibility layer). Runs `bob run -f stream-json --mode=<slug> --trust` with an automatically selected slug plus `BOB_EXTRA_ARGS`; never passes `--disable-subagents`. Task/review runs line-buffer assistant `message` text so a `<<<RALPHEX:...>>>` token split across streaming deltas is re-assembled into one `content_block_delta` (partial trailing line flushed at stream end); plan runs buffer the same deltas, validate and emit the first complete `QUESTION`/`PLAN_DRAFT`/`PLAN_READY`/`TASK_FAILED` boundary, then terminate Bob to suppress autonomous continuation. Missing or malformed plan boundaries fail closed. v2 schema handling: `isReasoning` replaces the v1 `<thinking>` heuristic (reasoning messages suppressed unless `BOB_VERBOSE=1`), `tool_result` errors read `.error.message` (`output` is absent on error), `result.status` is always `"success"` so `{type:"error"}` is the only failure channel — translated to `error: bob: <message>` with a forced non-zero exit. Because `ClaudeExecutor` ignores a non-zero provider exit once any signal was detected, and bob emits max-turns/max-cost aborts after assistant text, the error branch also emits `<<<RALPHEX:TASK_FAILED>>>` when a terminal signal was already forwarded (tracked by `bob_signal_emitted`, set in `emit_task_chunk` for answer text only) — `detectSignal` keeps the last match, so the failure supersedes the earlier completion. Deliberately order-sensitive: an error *followed* by a genuine signal is not retracted, and a signal-less error synthesizes nothing (the non-zero exit already fails it, and a signal would suppress `claude_retry_patterns`). Suppressed events emit empty keepalives; stdout and stderr are merged into one stream and non-JSON diagnostics are forwarded with `<<<RALPHEX:` neutralized. No model/effort forwarding: bob v2 stable removed `-m`/`--model`, so `--model`/`--model=`/`BOB_MODEL` and `--effort`/`--effort=` are accepted, noted once on stderr, and ignored. Headless tool access is governed ONLY by the active mode's `groups` list (`getToolsForMode()` -> `getToolsForGroups(groups)`) plus the always-passed `--trust` and bob's unconditional outside-workspace block; there is no approval preflight and no `BOB_SETTINGS_FILE` handling. bob's `approval.*` settings in `~/.bob/settings/settings.json` are NOT read by `bob run`: `ApprovalEngine` is reachable only via `ToolApprovalHandler.handleToolApproval`, whose single caller is the interactive TUI pending-tool reducer, and the handler is constructed only in the interactive session controller's `initialize()` — hence `--auto-approve` on `bob chat` but not `bob run`. Do not reintroduce an approval preflight or a settings-writing installer flag: it warns on every stock install for no headless benefit, and granting `autoApprovalEnabled: true` machine-wide silently disables the user's interactive `bob chat` confirmations. To narrow a phase's access, edit the mode's `groups`. The `error` event's `severity` is not inspected — every `{type:"error"}` fails the run, reading the top-level `message`, falling back to a nested `error.message`, and using an `unspecified bob error` placeholder only when both are absent or blank (so a cause that would match `claude_limit_patterns` is never discarded). Install `scripts/bob-as-claude/modes/` with `bash scripts/bob-as-claude/install-modes.sh` before automatic selection (the installer takes no arguments and rejects unknown ones); the installer targets `~/.bob/settings/custom_modes.yaml`, preserves Bob's legacy migration source when present, and lets project-level `.bob/custom_modes.yaml` override global modes. Valid v2 mode groups are only `read`, `edit`, `execute`, `mcp`, `skill`, `todo`, `subagent`, `mode` (invalid names silently grant nothing); task/plan use `read`/`edit`/`execute`, review adds `subagent` and instructs native parallel `spawn_subagent` — nested bob is blocked by bob's own `BOB_SESSION` check, so no guard shims exist. Subagent activity emits no stream events, so bob review phases want a generous or disabled `idle_timeout`. Review start markers select `ralphex-review`, the complete plan marker set selects `ralphex-plan`, and task/finalize prompts select `ralphex-task`; a non-empty `BOB_CHAT_MODE=<slug>` override always wins. Review and plan prompts are both passed unchanged (no wrapper-prepended adapter). `SHELL` is pinned to bash for the bob child (`SHELL="$bob_shell"` scoped to the invocation at `bob-as-claude.sh`, resolved from `BOB_SHELL` with `command -v`, default `bash`): bob's `execute_command` runs every command through `$SHELL` verbatim (`getShellPath()` is `platform==="win32" ? "powershell.exe" : process.env.SHELL`) and reports it to the model as `systemInfo.shell`, so a non-POSIX login shell like `fish` fails every heredoc / `VAR=$(...)` / `[[ ]]` / `$'...'` the model writes with exit 127. bob's `/bin/sh` fallback does NOT cover this — it fires only when the shell binary fails to launch (`exitCode === undefined`), not when the shell launches and rejects the script. The failure is not merely a retried command: an observed run's review phase could not heredoc its sub-agent prompt files and abandoned parallel review for a sequential self-review. Pinning also makes execution reproducible instead of varying with each user's `/etc/passwd` shell. It is not an override of user preference but an alignment with bob's own contract: bob's `execute_command` tool description hardcodes "The shell uses bash syntax." on every non-win32 host (`bashGuidelines()`) and its interactive `!command` path hardcodes bash, so the model is always told bash while only this one path leaks `$SHELL` — the pin is therefore unconditional rather than an "override only non-POSIX shells" allowlist, which would still break `dash`/`sh` users on `[[ ]]` and `$'...'`. Do not "simplify" this by exporting `SHELL` globally in the wrapper (the wrapper's own subshells should keep the caller's value) or by dropping it on the assumption bob defaults to `/bin/sh`. Transient-failure retry: `classify_bob_transient` sets `bob_transient_detected` from bob's non-JSON CLI diagnostics and its `{type:"error"}` message, and the wrapper emits the bare token `BOB_TRANSIENT_ERROR` (matched by the `claude_retry_patterns` default) as its last text when `bob_exit` is non-zero AND a transient cause was seen. Three constraints are load-bearing: (a) the classifier is NOT applied to `tool_result` errors — those carry arbitrary command output, so a build log containing "service unavailable" would forge a retry; bob's own diagnostics can never contain model prose because assistant text arrives as parsed `message` events, which is what makes plain-English matching safe there; (b) the marker must stay a bare token, never a `<<<RALPHEX:...>>>` form — signal-shaped text is neutralized on non-answer paths, and a real signal makes `result.Signal` non-empty, which suppresses retry detection entirely; (c) the non-zero-exit gate is required because bob can log a hiccup, recover, and finish, and marking that run transient would discard completed work. The classifier deliberately omits a bare `request failed` (bob wraps deterministic 4xx in it), 429/"too many requests" (a quota limit — `claude_limit_patterns`' job, needs `--wait`), and ECONNREFUSED/ENOTFOUND (near-always misconfiguration).
...
- `claude_retry_patterns`: comma-separated transient Claude/fya markers retried like executor timeouts. Default: `FYA_TRANSIENT_TIMEOUT,API Error: 529,API Error: 502,API Error: 503,API Error: 504,BOB_TRANSIENT_ERROR`. `BOB_TRANSIENT_ERROR` is emitted by `scripts/bob-as-claude/bob-as-claude.sh` (`classify_bob_transient`) — the wrapper classifies bob's own failure diagnostics and emits this opaque token only when bob exits non-zero from a transient cause, so the shared claude-wide default carries no bob-specific English phrase that review prose could trip. See the bob wrapper bullet for the classifier's boundaries. The transient HTTP errors (529 Overloaded, 502/503/504 gateway) live here, not in `claude_limit_patterns`: they are short-lived server hiccups, so they auto-retry without requiring `--wait` (500 is intentionally excluded — it can be a deterministic server failure, caught by the enumerated `API Error: 500` error pattern). Precedence is retry → limit → error, so a no-signal 529 matches the retry tier first; the claude error set enumerates only hard codes (`API Error: 400/401/403/404/413/429/500`), not the transient ones, so a signal-present 529 is no longer flagged as an error and the completed run's signal is honored
