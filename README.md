...
- `BOB_SHELL` - shell bob's `execute_command` runs commands through (default: `bash`). bob passes `$SHELL` through verbatim, so the wrapper pins bash rather than inheriting a login shell like `fish`, under which the bash a model writes fails
